...
if button1:
    ...
    try : 
        data= data[data["SieveSize(mm)"].str.strip().str.lower() != 'pan']
        data["SieveSize(mm)"] = data["SieveSize(mm)"].astype(float)
        data["MassRetained(gm)"] = data["MassRetained(gm)"].astype(float)
        if not mass:
            result_df, d, coeff = analyze_sieve(data)
        elif isinstance(mass,float) :
            result_df, d, coeff = analyze_sieve(data,total_mass=mass)
    except ValueError: 
        st.warning("Please make sure all inputs are valid numbers.")
    try:
        fig = plot_psd(result_df, d)
        st.plotly_chart(fig)
    except Exception as e :
        st.warning("Error occured while plotting graph")
